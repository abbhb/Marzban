...
export const SecurityWorkspace = () => {
  ...
  const blocksQuery = useQuery(
    ...
    () =>
      fetch<PageResult<IPBlock>>(
        pageUrl("/commerce/admin/security/blocks", queryState, { source })
      ),
    { keepPreviousData: true }
  );
  const securityQuery = useQuery(["commerce", "security-settings"], () =>
    fetch<PortalSecuritySettings>("/commerce/admin/security/settings")
  );
  useEffect(() => {
    if (securityQuery.data) setSecurity(securityQuery.data);
  }, [securityQuery.data]);
  const result = blocksQuery.data || {
    items: [],
    total: 0,
    page: list.page,
  ...
  const saveSecurity = async () => {
    ...
    try {
      const updated = await fetch<PortalSecuritySettings>(
        "/commerce/admin/security/settings",
        { method: "PUT", body: security }
      );
      setSecurity(updated);
      queryClient.setQueryData(["commerce", "security-settings"], updated);
      toast({
        title: t("commerce.securitySaved"),
        status: "success",
        position: "top",
      });
    } catch (error: any) {
  ...
  const createBlock = async () => {
    if (!network.trim() || !reason.trim()) {
      toast({
        title: t("commerce.blockFieldsRequired"),
        status: "error",
        position: "top",
      });
      return;
  ...
  const revoke = async (id: number) => {
    setRevokingId(id);
    try {
      await fetch(`/commerce/admin/security/blocks/${id}/revoke`, {
        method: "POST",
      });
      await queryClient.invalidateQueries(["commerce", "blocks"]);
      toast({
  ...
  return (
    <VStack align="stretch" spacing="4">
        ...
        <AccordionItem border="0">
          <AccordionButton px={{ base: 4, md: 6 }} py="5">
            <Box flex="1" textAlign="start">
              <Text fontWeight="750">{t("commerce.autoBlockPolicy")}</Text>
              <Text mt="1" fontSize="sm" color="fg.muted">
                {t("commerce.autoBlockHelp")}
              </Text>
            </Box>
            <AccordionIcon />
          </AccordionButton>
            ...
            {security && (
              <SimpleGrid columns={{ base: 1, md: 2, xl: 3 }} spacing="4">
                <FormControl display="flex" alignItems="center" gap="3">
                  <Switch
                    isChecked={security.auto_block_enabled}
                    onChange={(event) =>
                      setSecurity({
                        ...security,
                        auto_block_enabled: event.target.checked,
                      })
                    }
                  />
                  <FormLabel mb="0">{t("commerce.autoBlockEnabled")}</FormLabel>
                </FormControl>
                <FormControl>
                  <FormLabel>{t("commerce.loginFailureLimit")}</FormLabel>
                  <Input
                    type="number"
                    min="2"
                    max="100"
                    value={security.login_failure_limit}
                    onChange={(event) =>
                      setSecurity({
                        ...security,
                        login_failure_limit: Number(event.target.value),
                      })
                    }
                  />
                </FormControl>
                <FormControl>
                  <FormLabel>{t("commerce.loginWindowMinutes")}</FormLabel>
                  <Input
                    type="number"
                    min="1"
                    value={security.login_window_seconds / 60}
                    onChange={(event) =>
                      setSecurity({
                        ...security,
                        login_window_seconds: Number(event.target.value) * 60,
                      })
                    }
                  />
                </FormControl>
                <FormControl>
                  <FormLabel>
                    {t("commerce.registrationFailureLimit")}
                  </FormLabel>
                  <Input
                    type="number"
                    min="2"
                    max="100"
                    value={security.registration_failure_limit}
                    onChange={(event) =>
                      setSecurity({
                        ...security,
                        registration_failure_limit: Number(event.target.value),
                      })
                    }
                  />
                </FormControl>
                <FormControl>
                  <FormLabel>
                    {t("commerce.registrationWindowMinutes")}
                  </FormLabel>
                  <Input
                    type="number"
                    min="1"
                    value={security.registration_window_seconds / 60}
                    onChange={(event) =>
                      setSecurity({
                        ...security,
                ...
                <FormControl>
                  <FormLabel>{t("commerce.autoBlockHours")}</FormLabel>
                  <Input
                    type="number"
                    min="0"
                    value={security.auto_block_seconds / 3600}
                    onChange={(event) =>
                      setSecurity({
                        ...security,
                        auto_block_seconds: Number(event.target.value) * 3600,
                      })
                    }
                  />
                </FormControl>
      ...
      <DataWorkspace
        ...
        emptyTitle={
          list.search || list.status || source
            ? t("commerce.noMatchingBlocks")
            : t("commerce.noBlocks")
        }
        toolbar={
          <HStack
          ...
          >
            <WorkspaceSearch
              state={list}
              setState={setList}
              isLoading={blocksQuery.isFetching}
              placeholder={t("commerce.searchBlocks")}
            />
            <Select
              size="sm"
              w={{ base: "calc(50% - 8px)", md: "140px" }}
              rounded="xl"
              value={list.status}
              onChange={(event) =>
                setList((value) => ({
            ...
            >
              ...
              <option value="blocked">{t("commerce.blocked")}</option>
              <option value="expired">{t("commerce.expired")}</option>
              <option value="revoked">{t("commerce.revoked")}</option>
            </Select>
            <Select
              size="sm"
              w={{ base: "calc(50% - 8px)", md: "150px" }}
              rounded="xl"
              value={source}
              onChange={(event) => {
                setSource(event.target.value);
                setList((value) => ({ ...value, page: 1 }));
              }}
              aria-label={t("commerce.filterBySource")}
            >
              <option value="">{t("commerce.allSources")}</option>
              <option value="manual">{t("commerce.blockSource.manual")}</option>
              <option value="portal_login">
                {t("commerce.blockSource.portal_login")}
              </option>
              <option value="admin_login">
                {t("commerce.blockSource.admin_login")}
              </option>
              <option value="portal_registration">
                {t("commerce.blockSource.portal_registration")}
              </option>
            </Select>
            <IconButton
              size="sm"
      ...
      >
        <TableContainer>
          <Table size="sm" className="workspace-table" minW="980px">
            <Thead>
              <Tr>
                <Th>{t("commerce.ipOrCidr")}</Th>
                <Th>{t("commerce.status")}</Th>
                <Th>{t("commerce.source")}</Th>
                <Th>{t("commerce.blockReason")}</Th>
                <Th>{t("commerce.expiresAt")}</Th>
              ...
              {result.items.map((row) => {
                ...
                return (
                  <Tr key={row.id} className="workspace-row">
                    <Td>
                      <Code fontSize="xs">{row.network}</Code>
                    </Td>
                    <Td>
                      <Badge
                        colorScheme={state === "blocked" ? "red" : "gray"}
                        rounded="full"
                      >
                        {t(`commerce.${state}`)}
                      </Badge>
                    ...
                    <Td maxW="300px">
                      <Text noOfLines={2}>{row.reason}</Text>
                    </Td>
                    <Td>
                      {row.expires_at
                        ? dayjs
      ...
      >
        ...
        <ModalContent layerStyle="glass-strong" rounded="3xl" mx="4">
          <ModalHeader>{t("commerce.manualBlock")}</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <VStack align="stretch" spacing="4">
              <FormControl>
                <FormLabel>{t("commerce.ipOrCidr")}</FormLabel>
                <Input
                  placeholder="203.0.113.8 or 203.0.113.0/24"
                  value={network}
                  onChange={(event) => setNetwork(event.target.value)}
                />
              </FormControl>
              <FormControl>
                <FormLabel>{t("commerce.blockReason")}</FormLabel>
                <Input
                  value={reason}
              ...
              <FormControl>
                <FormLabel>{t("commerce.blockExpires")}</FormLabel>
                <Input
                  type="datetime-local"
                  value={expiresAt}
